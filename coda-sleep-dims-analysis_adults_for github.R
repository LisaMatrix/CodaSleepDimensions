...
dataset1<- subset.data.frame(data, favalidwkwe == 1)
...
dataset2 <- dataset1 %>% dplyr::select(hicid, ff02m2cp, season, fsep2, faagew,
                                       fpcodenw, fch23a12b, faa52sdtm,
                                       faa52ligtm, faa52mvtm, faa52slmp,
                                       faa52bedtm, faa52guptm, faa52slcon,
                                       faa52sltm, faa52sleff, fch23a07d)

##Checking correct variables
str(dataset1$fch23a07d)
str(dataset1$fch23a12b)
str(dataset1$ff02m2cp)

##Rename variables
dataset2$MVPA <- dataset2$faa52mvtm
dataset2$Sleep <- dataset2$faa52sltm
dataset2$Sedentary <- dataset2$faa52sdtm
dataset2$LPA <- dataset2$faa52ligtm
...
dataset$comp4 <- dataset %>% dplyr::select(faa52sltm, faa52sdtm, faa52mvtm, faa52ligtm)
...
##wear time
mean(dataset$faa52sltm+ dataset$faa52sdtm+ dataset$faa52mvtm+ dataset$faa52ligtm)
sd(dataset$faa52sltm+ dataset$faa52sdtm+ dataset$faa52mvtm+ dataset$faa52ligtm)

#### Descriptive table####

##need to get BMI
dataset2b<-dataset1 %>% dplyr::select (hicid, fabmicp)
...
Table_adult<- Table_dd %>% dplyr::select (!hicid)
Table_adult$zf02m1cp <-as.factor(Table_adult$ff02m2cp)
Table_adult$fch23a07d <-as.factor(Table_adult$fch23a07d)
Table_adult$fch23a12b <-as.factor(Table_adult$fch23a12b)
Table_adult$faagew  <-as.numeric(Table_adult$faagew)


##Create table
Table_a<-tableone::CreateTableOne(data=Table_adult)
Table_a


#### Analysis #####
is.numeric(dataset$faagew)
is.numeric(dataset$ff02m2cp)
dataset$ff02m2cp<-as.factor(dataset$ff02m2cp)
...
dataset$TroubledSleep <- as.numeric(dataset$fch23a12b)
delta_mins <- seq(-30, 30, by = 15)
comp_names <- c("MVPA", "Sleep", "Sedentary", "LPA")
cov_names <- c("fsep2", "ff02m2cp", "faagew", "season")
...
### set variables to use in codaredistlm package
str(dataset$fch23a07d)
dataset$Tired <- as.numeric(dataset$fch23a07d)
delta_mins <- seq(-30, 30, by = 15)
comp_names <- c("MVPA", "Sleep", "Sedentary", "LPA")
cov_names <- c("fsep2", "ff02m2cp", "faagew", "season")
outc_name <- "Tired"
...
var_adf_fig1 <- as_tibble(deltacomp_adf_variability) %>% mutate(outcome = "Sleep Variability")
